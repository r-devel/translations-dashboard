# Data Files

This folder contains the data used for the translation dashboards.

## File Descriptions

- **library_language_statistics.csv**  
  Translation status by package and language.

- **language_statistics.csv**  
  Data about all supported languages, including their identifiers, translation progress metrics, and the timestamp of the last update.

- **message_status.csv**  
  The current status of each message, by language.

- **marked_for_edit.csv**  
  Messages flagged in Weblate as needing edits.

- **metadata.csv**  
  Metadata for each translation file.

- **new_translation.csv**  
  Recent translation activity by user and message, with timestamps.

- **statistics.csv**  
  List of translators and their contribution statistics.
