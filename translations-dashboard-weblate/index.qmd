...
format:
  dashboard:
    ...
    nav-buttons:
      - github
      - icon: github
        href: https://github.com/r-devel/translations-dashboard
---

```{r setup, include=FALSE}
source("R/packages.R")    # loads packages
source("R/load_data.R")   # loads data, custom functions and global translations 
```
...
# Languages {orientation="columns" scrolling="true"}
...
p <- ggplot(df_long, aes(x = Name, y = value, fill = variable)) +
  ...
  scale_fill_manual(values = c("Translated" = "#A2FF86", "Fuzzy" = "#0079FF", "Untranslated" = "#F31559")) #+
  # theme(
  ...
  #   legend.background=element_rect(fill="pink", size=1, linetype="solid")
  #   )
p
# p_plotly <- ggplotly(p)
```

This graph depicts the translation status of each language.
...
# Leaderboard {orientation="columns" scrolling="true"}
...
# Library {orientation="columns" scrolling="true"}
...
# Translations {orientation="columns" scrolling="true"}
...
linked <- rep(NA, nrow(Marked_for_Edit))  # pre-fill with NA

for (i in seq_len(nrow(Marked_for_Edit))) {

  u <- Marked_for_Edit$units[i]

  url <- paste0("https://translate.rx.studio/api/units/", u, "/")
  h <- curl::new_handle()
  handle_setopt(h, ssl_verifyhost = 0L, ssl_verifypeer = 0L)
  handle_setopt(h, customrequest = "GET")
  handle_setopt(h, httpheader = c(
    "Authorization: Token wlu_U8k6Kk12pyhXuBeXOP6imHRFiPrUMwHgHari"
  ))
  ...
  if (!is.null(users_last$web_url)) {
    linked[i] <- users_last$web_url
  }
}

data <- Marked_for_Edit
data$linked <- linked
data$date <- as.Date(data$date)
...
# Information {orientation="columns" scrolling="true"}
...
# R Translation Teams {orientation="columns" scrolling="true"}
